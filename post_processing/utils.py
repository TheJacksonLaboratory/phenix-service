...
EXCEL_COLS = list(ascii_uppercase) + [
    "".join(x) for x in product(ascii_uppercase, repeat=2)
]
...
def convert_96_to_384(data_96, name, quad=None, colwise=False):
    ...
    if quad is None:
        index = pd.Index(
            sum([index_by_quad(k, colwise).tolist() for k in range(4)], [])
        )
